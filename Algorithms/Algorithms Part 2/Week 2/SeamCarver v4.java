    ...
    public SeamCarver(Picture picture)
    {
 	   ...
 	   for (int col = 0; col < width; col++)
	    	for (int row = 0; row < height; row++)
	    		map[col + picture.width() * row].RGB = picture.getRGB(col, row);
 	    for (int col = 0; col < width; col++)
 	    	for (int row = 0; row < height; row++)
...
/*    	for (int col = 0; col < width; col++)
    	...
    	for (int col = 0; col < width; col++)
            for (int row = 0; row < height; row++)
            	result.setRGB(col, row, pixel(col, row).RGB);
	    return result;
    }
    ...
    private void calculate_energy(int x, int y)
    {
	    ...
	    {
	    	map[x + picture.width() * y].energy = border_energy;
	    	return;
	    }
/*	    
	    ...
	    int cx1 = pixel(x - 1, y).RGB;
	    int cx2 = pixel(x + 1, y).RGB;
	    int cy1 = pixel(x, y - 1).RGB;
	    int cy2 = pixel(x, y + 1).RGB;
	    
	    map[x + picture.width() * y].energy = Math.sqrt(
    ...
    { 
	    ...
	    for (int x = 0; x < width; x++)
	    	for (int y = seam[x]; y < height; y++)
	    		map[x + picture.width() * y] = pixel(x, y + 1);
    ...
    public void removeVerticalSeam(int[] seam)
    {
	    ...
	    {
	    	int pos = seam[y] + picture.width() * y;
	    	System.arraycopy(map, pos + 1, map, pos, picture.width() - seam[y] - 1);
	    } 
