...
public class SeamCarver 
{
    ...
    public SeamCarver(Picture picture)
    {
 	   ...
 	   for (int col = 0; col < width; col++)
	    	for (int row = 0; row < height; row++)
	    		map[col + picture.width() * row] = picture.getRGB(col, row);
    }
    ...
    public Picture picture()
    {
    	...
    	for (int col = 0; col < width; col++)
            for (int row = 0; row < height; row++)
            	result.setRGB(col, row, pixel(col, row));
	    return result;
    }
    ...
    public int[] findHorizontalSeam()
    {
    	...
    	for (int col = 0; col < width - 2; col++)
    		for (int row = 0; row < height - 2; row++)
    			energy[col + (width - 2) * row] = energy(col + 1, row + 1);
    	
    	for (int y = 0; y < height - 2; y++)
    		distTo[(width - 2) * y] = energy[(width - 2) * y];
    	...
    	for (int x = 0; x < width - 4; x++) // relaxing inner vertices
    		for (int y = 0; y < height - 2; y++) 
    	    	relax_horizontal(x, y, distTo, edgeTo, energy);
    	...
    	else
    	{
    		for (int y = 0; y < height - 2; y++) // relaxing border vertices
    		{
    			int v = relax_horizontal(width - 4, y, distTo, edgeTo, energy);
    			if (min > distTo[v])
    			{
    				min = distTo[v];
    				end = v;
    ...
    private int relax_horizontal(int x, int y, double[] distTo, int[] edgeTo, double[] energy)
    {
    	double min = Double.POSITIVE_INFINITY;
    	int min_ = 0;
    	for (int i = - 1; i < 2; i++)
    	{
    		...
    		int v = x + (width - 2) * y;
    		int w = v + 1 + (width - 2) * i;
    		if (distTo[w] > distTo[v] + energy[x + 1 + (width - 2) * (y + i)]);
    		{
    			distTo[w] = distTo[v] + energy[x + 1 + (width - 2) * (y + i)];
    			edgeTo[w] = v;
    		}
    		if (distTo[w] < min) 
    		{
    ...
    public int[] findVerticalSeam()
    {
    	...
    	for (int y = 0; y < height - 4; y++) // relaxing inner vertices
    		for (int x = 0; x < width - 2; x++) 
    	    	relax_vertical(x, y, distTo, edgeTo, energy);
    	...
    	else
    	{
    		for (int x = 0; x < width - 2; x++) // relaxing border vertices
    		{
    			int v = relax_vertical(x, height - 4, distTo, edgeTo, energy);
    			if (min > distTo[v])
    			{
    				min = distTo[v];
    				end = v;
    ...
    private int relax_vertical(int x, int y, double[] distTo, int[] edgeTo, double[] energy)
    {
    	double min = Double.POSITIVE_INFINITY;
    	int min_ = 0;
    	for (int i = - 1; i < 2; i++)
    	{
    		...
    		int v = x + (width - 2) * y;
    		int w = v + (width - 2) + i;
    		if (distTo[w] > distTo[v] + energy[x + i + (width - 2) * (y + 1)])
    		{
    			distTo[w] = distTo[v] + energy[x + i + (width - 2) * (y + 1)];
    			edgeTo[w] = v;
    		}
    		if (distTo[w] < min) 
    		{
